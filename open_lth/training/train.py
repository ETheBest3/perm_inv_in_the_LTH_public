...
def train(
    ...
    #Preload all data on gpu
    #batch_list = []

    #for i, batch in enumerate(train_loader):
    #    batch = batch.to(device=get_platform().torch_device)
    #    # Append the data to the list
    #    batch_list.append(batch)
    
    # The training loop.
    for ep in tqdm(range(start_step.ep, end_step.ep + 1)):
        ...
        for it, batch in enumerate(train_loader):
            if isinstance(model, gcn.Model):
                x = batch.x
                edge_index = batch.edge_index
                y = batch.y
            elif isinstance(model, pruned_model.PrunedModel) and isinstance(model.model, gcn.Model):
                x = batch.x
                edge_index = batch.edge_index
                y = batch.y
            else:
            ...
            # Otherwise, train.
            if isinstance(model, gcn.Model):
                x = x.to(device=get_platform().torch_device)
                edge_index = edge_index.to(device=get_platform().torch_device)
                y = y.to(device=get_platform().torch_device)
            elif isinstance(model, pruned_model.PrunedModel) and isinstance(model.model, gcn.Model):
                x = x.to(device=get_platform().torch_device)
                edge_index = edge_index.to(device=get_platform().torch_device)
                y = y.to(device=get_platform().torch_device)
            ...
            if isinstance(model, gcn.Model):
                if isinstance(model.loss_criterion, torch.nn.BCELoss):
                    loss = model.loss_criterion(model(x, edge_index).squeeze(), y.float())
                else:
                    loss = model.loss_criterion(model(x, edge_index), y)
            elif isinstance(model, pruned_model.PrunedModel) and isinstance(model.model, gcn.Model):
                if isinstance(model.loss_criterion, torch.nn.BCELoss):
                    loss = model.loss_criterion(model(x, edge_index).squeeze(), y.float())
                else:
