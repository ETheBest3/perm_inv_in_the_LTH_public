...
from pruning import pruned_model
...
def create_eval_callback(eval_name: str, loader: DataLoader, verbose=False):
    ...
    def eval_callback(output_location, step, model, optimizer, logger):
        ...
        def correct(labels, outputs):
            if isinstance(model.loss_criterion, torch.nn.BCELoss):
                predicted = outputs > 0.5
                return torch.sum(predicted == labels).item()
            else:
        ...
        with torch.no_grad():
            for batch in loader:
                if isinstance(model, gcn.Model) or (isinstance(model, pruned_model.PrunedModel) and isinstance(model.model, gcn.Model)):
                    x = batch.x
                    edge_index = batch.edge_index
                    y = batch.y
                    x = x.to(get_platform().torch_device)
                    edge_index = edge_index.to(get_platform().torch_device)
                    y = y.squeeze().to(get_platform().torch_device)
                    output = model(x, edge_index)

                    labels_size = torch.tensor(len(y), device=get_platform().torch_device)
                    
                    example_count += labels_size
                    if isinstance(model.loss_criterion, torch.nn.BCELoss):
                        total_loss += model.loss_criterion(output.squeeze(), y.float()) * labels_size
                        total_correct += correct(y.float(), output.squeeze())
                    else:
                        total_loss += model.loss_criterion(output, y) * labels_size
                        total_correct += correct(y, output)
                else:
...
def standard_callbacks(training_hparams: hparams.TrainingHparams, train_set_loader: DataLoader,
    ...
    #When wanting to also save the model
    result = [
