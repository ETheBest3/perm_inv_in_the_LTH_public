...
from datasets import base, cifar10, mnist, imagenet, graph_dataset
from foundations.hparams import DatasetHparams
from platforms.platform import get_platform

registered_datasets = {'cifar10': cifar10, 'mnist': mnist, 'imagenet': imagenet, 'graph': graph_dataset}
...
def get(dataset_hparams: DatasetHparams, train: bool = True):
    ...
    if dataset_hparams.dataset_name in registered_datasets:
        ...
        if train:
            if dataset_hparams.dataset_name == 'graph':
                dataset = graph_dataset.Dataset.get_train_set(dataset_hparams.root, dataset_hparams.num_graphs, dataset_hparams.num_nodes, 
                                                              dataset_hparams.k, dataset_hparams.num_class, dataset_hparams.num_colors, 
                                                              dataset_hparams.dist)
            else:
                dataset = registered_datasets[dataset_hparams.dataset_name].Dataset.get_train_set(use_augmentation)
        else:
            if dataset_hparams.dataset_name == 'graph':
                dataset = graph_dataset.Dataset.get_test_set(dataset_hparams.root, dataset_hparams.num_graphs, dataset_hparams.num_nodes, 
                                                              dataset_hparams.k, dataset_hparams.num_class, dataset_hparams.num_colors, 
                                                              dataset_hparams.dist)
            else:
...
def iterations_per_epoch(dataset_hparams: DatasetHparams):
    ...
    if dataset_hparams.dataset_name in registered_datasets:
        if dataset_hparams.dataset_name == 'graph':
            num_train_examples = dataset_hparams.num_graphs*0.8
        else:
...
def num_classes(dataset_hparams: DatasetHparams):
    ...
    if dataset_hparams.dataset_name in registered_datasets:
        if dataset_hparams.dataset_name == 'graph':
            num_classes = dataset_hparams.num_class
        else:
