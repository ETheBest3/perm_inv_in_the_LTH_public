...
from models import cifar_resnet, cifar_vgg, mnist_lenet, imagenet_resnet, gcn
from models import bn_initializers, initializers
from platforms.platform import get_platform

registered_models = [mnist_lenet.Model, cifar_resnet.Model, cifar_vgg.Model, imagenet_resnet.Model, gcn.Model]
...
def get(model_hparams: ModelHparams, outputs=None):
    ...
    for registered_model in registered_models:
        if registered_model.is_valid_model_name(model_hparams.model_name):
            if isinstance(registered_model, gcn.Model):
                model = registered_model.get_model_from_name(model_hparams.model_name, init_fn, outputs=None, criterion=model_hparams.loss_criterion)
            else:
